...
#ifdef CONFIG_QCOM_RFS
#if defined(CONFIG_VLAN_8021Q) || defined(CONFIG_VLAN_8021Q_MODULE)
#include <linux/if_vlan.h>
#endif
#include <linux/qca-rfs/rfs_dev.h>
#include "fal_rfs.h"
#endif
...
#ifdef RKS_KERNEL
	fal_fdb_port_learn_set(dev_id,1,A_FALSE);
	fal_fdb_port_learn_set(dev_id,5,A_FALSE);
	fal_fdb_port_learn_set(dev_id,2,A_FALSE);
	fal_fdb_port_learn_set(dev_id,3,A_FALSE);
	fal_fdb_port_learn_set(dev_id,4,A_FALSE);
#endif
	return SW_OK;
}
...
#if 1
				printk(KERN_ALERT "tx_ok = 0x%x, rx_ok = 0x%x, tx_counter_error = 0x%x, rx_counter_error = 0x%x\n",
						tx_ok, rx_ok, tx_counter_error, rx_counter_error);
				printk(KERN_ALERT "PHY %d single test PSGMII issue happen \n", phy);
#endif
				phy_t_status |= (1<<phy);
			}
...
void ssdk_psgmii_self_test()
{
	...
	for(i = 0; i < 100; i++) {
		...
		//for (phy = 0;phy < 5; phy++) {
			ssdk_psgmii_single_phy_testing(0);
			ssdk_psgmii_single_phy_testing(4);
//		}
...
static ssize_t proc_write_rdsw(struct file *filp, const char __user *buffer,
				size_t buffer_length, loff_t *pos)
{
    unsigned char curr_reg_info[64];
    unsigned short len, reg;
    int i, retval;
    char *pEnd, *pBegin;
	a_uint32_t reg_value;
	struct qca_phy_priv *priv = RKSV_PDE_DATA(filp->f_path.dentry->d_inode);
	...
	qca_switch_reg_read(0, reg, (a_uint8_t *)&reg_value, 4);
	pr_crit("%X\n", reg_value);
    
    return buffer_length;
}

static ssize_t proc_read_rdsw(struct file *filp, char __user *buffer,
...
static ssize_t proc_write_wrsw(struct file *filp, const char __user *buffer,
				size_t buffer_length, loff_t *pos)
{
    unsigned char curr_reg_info[64];
    unsigned short len, reg;
	...
	qca_switch_reg_write(0, reg, (a_uint8_t *)&reg_value, 4);

    return buffer_length;
}

static ssize_t proc_read_wrsw(struct file *filp, char __user *buffer,
			size_t count, loff_t *pos)
{
    return 0;
}

static int proc_show_rdsw_all(struct seq_file *m, void *v)
{
    int i;
	a_uint32_t reg_value;
	struct qca_phy_priv *priv = m->private;

    for (i=0; i<=0xe0; i+=4) {
		qca_switch_reg_read(0, i, (a_uint8_t *)&reg_value, 4);
		seq_printf(m, "%4X -- %8X\n", i, reg_value);
    }
	seq_putc(m, '\n');
    for (i=0x400; i<=0x474; i+=4){
		qca_switch_reg_read(0, i, (a_uint8_t *)&reg_value, 4);
		seq_printf(m, "%4X -- %8X\n", i, reg_value);
    }
	seq_putc(m, '\n');
    for (i=0x600; i<=0x718; i+=4){
		qca_switch_reg_read(0, i, (a_uint8_t *)&reg_value, 4);
		seq_printf(m, "%4X -- %8X\n", i, reg_value);
    }
...
static int proc_open_rdsw_all(struct inode *inode, struct file *file)
{
  return single_open(file, proc_show_rdsw_all, RKSV_PDE_DATA(inode));
}

//------------------------------------------------------------------------
#define RDSW_FILE    "rdsw"
#define WRSW_FILE    "wrsw"
#define RDSW_ALL_FILE    "rdsw_all"

static struct proc_dir_entry *proc_enet = NULL;

#define PROC_NET_SWITCH   "switch"
...
static void rks_ssdk_switch_create_proc_entries(struct qca_phy_priv *priv)
{
    if ((proc_enet = proc_mkdir(PROC_NET_SWITCH, init_net.proc_net)) == NULL ){
        printk("%s: proc_mkdir(%s,0)\n failed\n", __FUNCTION__, PROC_NET_SWITCH);
        return;
    } else {
        printk("%s: proc_enet->name=%s proc_enet->parent->name=%s \n",
            __func__, PROC_NET_SWITCH, "net");
    }

    if (!rks_create_rdwr_proc(&fops_eth_rdsw, RDSW_FILE,
  					proc_read_rdsw, proc_write_rdsw, NULL, priv, proc_enet)) {
        printk("%s: create_proc_entry(%s/%s) failed\n", __FUNCTION__,
                PROC_NET_SWITCH, RDSW_FILE);
    }

    if (!rks_create_rdwr_proc(&fops_eth_wrsw, WRSW_FILE,
  					proc_read_wrsw, proc_write_wrsw, NULL, priv, proc_enet)) {
        printk("%s: create_proc_entry(%s/%s) failed\n", __FUNCTION__,
                PROC_NET_SWITCH, WRSW_FILE);
    }

	/* only dump some VLAN related registers */
    if (!rks_create_read_proc(&fops_eth_rdsw_all, RDSW_ALL_FILE,
  					NULL, proc_open_rdsw_all, priv, proc_enet)) {
        printk("%s: create_proc_entry(%s/%s) failed\n", __FUNCTION__,
                PROC_NET_SWITCH, RDSW_ALL_FILE);
    }

    return;
}

static void rks_ssdk_switch_cleanup_proc_entries(void)
{

    if ( proc_enet == NULL ) return;

    remove_proc_entry(RDSW_ALL_FILE, proc_enet);
    remove_proc_entry(WRSW_FILE, proc_enet);
    remove_proc_entry(RDSW_FILE, proc_enet);
}

static void rks_ssdk_init_fixup(struct switch_dev *dev)
{
#if 1 /* RKS_KERNEL_SW_PORT_BASED_MODE */
	/* Port-based VLAN mode, isolate all ports except port0(cpu port) */
    struct qca_phy_priv *priv = qca_phy_priv_get(dev);
    priv->vlan = 0;
    priv->init = true;
    dev->ops->apply_config(dev); /* see qca_ar8327_sw_hw_apply() */
#endif

	rks_ssdk_switch_create_proc_entries(priv);
}

static void rks_ssdk_exit_fixup(void)
{
    rks_ssdk_switch_cleanup_proc_entries();
}
#endif
...
#ifdef RKS_KERNEL
		rks_ssdk_init_fixup(&qca_phy_priv_global->sw_dev);
#endif
	}else{
